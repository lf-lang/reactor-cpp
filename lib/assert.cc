...
void assert_phase_debug_print([[maybe_unused]] Phase expected_phase, [[maybe_unused]] Phase actuall_phase) {
  ...
  // C++20 std::format
  throw ValidationError("Expected Phase: " + enum_value_to_name(expected_phase) +
                        " Current Phase: " + enum_value_to_name(actuall_phase));
}
...
void assert_phase([[maybe_unused]] const ReactorElement* ptr, [[maybe_unused]] Phase phase) {
  if constexpr (runtime_assertion) { // NOLINT
    if (ptr->environment()->phase() != phase) {
      assert_phase_debug_print(phase, ptr->environment()->phase());
    }
  }
}
...
void assert_phase([[maybe_unused]] const Environment* ptr, [[maybe_unused]] Phase phase) {
  if constexpr (runtime_assertion) { // NOLINT
    if (ptr->phase() != phase) {
      assert_phase_debug_print(phase, ptr->phase());
    }
  }
}
