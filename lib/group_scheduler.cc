...
#include "reactor-cpp/group_scheduler.hh"
...
thread_local const GroupWorker* GroupWorker::current_worker = nullptr;

GroupWorker::GroupWorker(GroupWorker&& work) // NOLINT(performance-noexcept-move-constructor)
...
void GroupWorker::work() const {
  // initialize the current worker thread local variable
  current_worker = this;

  log::Debug() << "(GroupWorker " << this->identity_ << ") Starting";

  if (identity_ == 0) {
    log::Debug() << "(GroupWorker 0) do the initial scheduling";
    scheduler_.schedule();
  }
  ...
  log::Debug() << "(GroupWorker " << identity_ << ") terminates";
}

void GroupWorker::execute_reaction(Reaction* reaction) const {
  log::Debug() << "(GroupWorker " << identity_ << ") "
...
auto GroupReadyQueue::pop() -> Reaction* {
  auto old_size = size_.fetch_sub(1, std::memory_order_acq_rel);

  // If there is no ready reaction available, wait until there is one.
  while (old_size <= 0) {
    log::Debug() << "(GroupWorker " << GroupWorker::current_worker_id() << ") Wait for work";
    sem_.acquire();
    log::Debug() << "(GroupWorker " << GroupWorker::current_worker_id() << ") Waking up";
...
void GroupReadyQueue::fill_up(std::vector<Reaction*>& ready_reactions) {
...
void GroupScheduler::terminate_all_workers() {
  log::Debug() << "(GroupScheduler) Send termination signal to all workers";
...
auto GroupScheduler::schedule_ready_reactions() -> bool {
  ...
  log::Debug() << "(GroupScheduler) Scanning the reaction queue for ready reactions";
  ...
  for (; reaction_queue_pos_ < reaction_queue_.size(); reaction_queue_pos_++) {
    ...
    if (!reactions.empty()) {
      log::Debug() << "(GroupScheduler) Process reactions of priority " << reaction_queue_pos_;
      ...
      if constexpr (log::debug_enabled || tracing_enabled) { // NOLINT
        for (auto* reaction : reactions) {
          log::Debug() << "(GroupScheduler) Reaction " << reaction->fqn() << " is ready for execution";
  ...
  log::Debug() << "(GroupScheduler) Reached end of reaction queue";
  return false;
}
...
void GroupScheduler::next() { // NOLINT
...
GroupScheduler::GroupScheduler(Environment* env)
    : using_workers_(env->num_workers() > 1)
    , environment_(env)
    , ready_queue_(env->num_workers()) {}

GroupScheduler::~GroupScheduler() = default;

void GroupScheduler::schedule_sync(const Tag& tag, BaseAction* action, std::function<void(void)> pre_handler) {
...
void GroupScheduler::schedule_async(const Tag& tag, BaseAction* action, std::function<void(void)> pre_handler) {
...
void GroupScheduler::set_port(BasePort* port) {
  ...
  set_ports_[GroupWorker::current_worker_id()].push_back(port);

  // recursively search for triggered reactions
  set_port_helper(port);
}

void GroupScheduler::set_port_helper(BasePort* port) {
  for (auto* reaction : port->triggers()) {
    triggered_reactions_[GroupWorker::current_worker_id()].push_back(reaction);
  }
