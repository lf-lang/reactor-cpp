...
void Environment::build_dependency_graph(Reactor *reactor) { //NOLINT
  ...
  for (auto *reaction: reactor->reactions()) {
    ...
    validate(
        result.second,
        "priorities must be unique for all reactions_ of the same reactor");
  }
...
void Environment::dump_instance_to_yaml(std::ofstream &yaml,
...
void Environment::dump_reaction_to_yaml(std::ofstream &yaml,
