...
template<class T>
reactor::MutationChangeBankSize<T>::MutationChangeBankSize(std::vector<T>* bank, Reactor* reactor, std::size_t size, std::function<T(Reactor*, std::size_t)> create_lambda)
    : bank_(bank), reactor_(reactor), desired_size_(size), create_lambda_(create_lambda){
}

template<class T>
void reactor::MutationChangeBankSize<T>::change_size(std::size_t new_size) {
...
template<class T>
auto reactor::MutationChangeBankSize<T>::run() -> MutationResult {
...
template <class T>
auto reactor::MutationChangeBankSize<T>::rollback() -> MutationResult {
