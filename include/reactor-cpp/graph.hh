...
public:
  ...
  [[nodiscard]] auto get_edges() const noexcept -> std::vector<std::tuple<E, P, E>> {
    std::vector<std::tuple<E, P, E>> all_edges{};
    for (auto const& [source, sinks] : graph_) {

      for (const auto& sink : sinks) {
        all_edges.emplace_back(source, sink.first, sink.second);
