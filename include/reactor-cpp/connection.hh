...
template <class T> class [[maybe_unused]] Connection : public Action<T> {
private:
...
public:
  ...
  void trigger() noexcept { this->schedule(incoming_port_interface_->get(), delay_); }

  void setup() noexcept override {
    // TODO: Action<T>::setup(); needed ?
    if constexpr (std::is_same<T, void>::value) {
      if (outgoing_port_interface_ != nullptr && outgoing_port_interface_ != nullptr &&
          incoming_port_interface_->is_present()) {
