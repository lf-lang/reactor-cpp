...
// this class is used inside the scope of the reaction and restricts modification
template <class A>
class RestrictedMultiport : private Multiport<A> {
  using allocator_type = A;
  using value_type = typename A::value_type;
  using reference = typename A::reference;
  using const_reference = typename A::const_reference;
  ...
  using const_iterator = typename std::vector<A>::const_iterator;

  RestrictedMultiport() : Multiport<A>() {}
  ~RestrictedMultiport() noexcept = default;

  using Multiport<A>::operator==;
  using Multiport<A>::operator!=;
  using Multiport<A>::operator[];
  using Multiport<A>::begin;
  using Multiport<A>::cbegin;
  using Multiport<A>::end;
  using Multiport<A>::cend;
  using Multiport<A>::size;
  using Multiport<A>::empty;
  using Multiport<A>::get_present_port_indices;
};
